...
int main() {
	...
	for (int i = 0; i < DATA_SIZE; i++)input_raw_ppg[i] = (float)AFEData1[i + PRE_DISCARDED_DATA_SIZE];
	...
	for (int i = 0; i < DATA_SIZE; i++)
	{
		...
		float x = (float)Accelerometer_X[i + PRE_DISCARDED_DATA_SIZE];
		float y = (float)Accelerometer_Y[i + PRE_DISCARDED_DATA_SIZE];
		float z = (float)Accelerometer_Z[i + PRE_DISCARDED_DATA_SIZE];
